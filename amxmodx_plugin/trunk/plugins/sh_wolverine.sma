...
new gPcvarHealPoints, gPcvarKnifeMult
...
public plugin_init()
{
	...
	gPcvarKnifeMult = register_cvar("wolv_knifemult", "1.35")
...
public client_damage(attacker, victim, damage, wpnindex, hitplace)
{
	...
	if ( gHasWolverine[attacker] && wpnindex == CSW_KNIFE ) {
		new headshot = hitplace == 1 ? 1 : 0

		// do extra damage
		new extraDamage = floatround(damage * get_pcvar_float(gPcvarKnifeMult) - damage)
		if ( extraDamage > 0 ) sh_extra_damage(victim, attacker, extraDamage, "knife", headshot)
	}
}
//----------------------------------------------------------------------------------------------
#if defined USE_WPN_MODEL
